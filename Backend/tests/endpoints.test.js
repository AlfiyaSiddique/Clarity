...
test('POST /api/signup - Create a new user', async () => {
  ...
  test('GET /api/notes/get/user - Get all notes of a user', async () => {
    await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .expect(200); // Assuming 200 is the status code for successful retrieval
  });
  ...
  test('PUT /api/notes/update/:noteid - Update a note', async () => {
    ...
    await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .send(updatedContent)
      .expect(200); // Assuming 200 is the status code for successful update
  });
  ...
  test('DELETE /api/notes/delete/:noteid - Delete a note', async () => {
    await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .expect(200); // Assuming 200 is the status code for successful deletion
  });
});
